...
export function HallsList() {
  ...
  const handleSetActiveHall = (hallId: string) => {
  ...
  return (
    <>
      <section className="lg:col-span-4 space-y-4">
        ...
        {halls.length === 0 ? (
          <div className="rounded-3xl border-2 border-dashed border-indigo-100 bg-indigo-50/30 text-center">
            <p className="text-indigo-900 font-semibold mb-4">
              No Halls found.
            </p>
            <button
              onClick={() => setShowCreateHall(true)}
              className="text-sm font-bold text-indigo-600 flex items-center gap-2 mx-auto"
            >
          ...
          <div className="bg-white rounded-4xl p-8 w-full max-w-md shadow-2xl">
            ...
            <div className="flex gap-3">
              ...
              <button
                onClick={() => {
                  if (newHallName) {
                    const hallId = createHall(newHallName);
                    setLastActiveHallId(hallId);
                    setNewHallName("");
